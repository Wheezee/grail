...
<nav class="mb-6" aria-label="Breadcrumb">
  <ol class="flex flex-wrap items-center gap-1 sm:gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400">
    ...
    <li class="flex items-center">
      <i data-lucide="chevron-right" class="w-3 h-3 sm:w-4 sm:h-4 mx-1 sm:mx-2 flex-shrink-0"></i>
      <span class="text-gray-900 dark:text-gray-100 font-medium whitespace-nowrap">Projects</span>
    </li>
  </ol>
</nav>
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
  <div>
    <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Projects — {{ $classSection->section }}</h2>
...
@if($projects->count() > 0)
  ...
  <div class="mb-6">
    <div class="flex flex-wrap gap-2 border-b border-gray-200 dark:border-gray-700">
      @foreach($projects as $project)
        <div class="px-0 py-0 text-sm font-medium rounded-t-lg transition-all duration-200 flex items-center group {{ $selectedProject && $selectedProject->id === $project->id ? 'bg-evsu text-white' : 'text-gray-600 dark:text-gray-400 hover:text-evsu dark:hover:text-evsu hover:bg-gray-100 dark:hover:bg-gray-700' }}">
          <a href="{{ route('projects.show', ['subject' => $classSection->subject->id, 'classSection' => $classSection->id, 'project' => $project->id]) }}" class="flex-1 px-4 py-2 flex items-center min-w-0">
  ...
  @if($selectedProject)
      ...
      <div class="flex flex-wrap items-center justify-between gap-6">
        <div class="flex flex-wrap items-center gap-6 text-sm">
          <div class="flex items-center gap-2">
            ...
            <span class="text-gray-900 dark:text-gray-100">{{ $selectedProject->max_score }}</span>
          </div>
          @if($selectedProject->hasDueDate())
            <div class="flex items-center gap-2">
              <span class="font-medium text-gray-700 dark:text-gray-300">Due Date:</span>
              <span class="text-gray-900 dark:text-gray-100 {{ $selectedProject->isOverdue() ? 'text-red-600 dark:text-red-400 font-semibold' : '' }}">
                {{ $selectedProject->due_date->format('F j, Y') }}
          ...
          @if($selectedProject->description)
            <div class="flex items-center gap-2">
              ...
              <span class="text-gray-900 dark:text-gray-100">{{ $selectedProject->description }}</span>
            </div>
          @endif
        </div>
        <div class="flex items-center gap-2 ml-auto">
          <button onclick="openEditProjectModal()" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg shadow transition-transform transform hover:scale-105 focus:outline-none h-10 bg-evsu text-white hover:bg-evsuDark">
            <i data-lucide="edit-3" class="w-4 h-4"></i>
            Edit
          </button>
          <button
            onclick="if(confirm('Delete this project? This action cannot be undone.')) { document.getElementById('delete-project-form-{{ $selectedProject->id }}').submit(); }"
          ...
          <form id="delete-project-form-{{ $selectedProject->id }}"
                method="POST"
                action="{{ route('projects.destroy', ['subject' => $classSection->subject->id, 'classSection' => $classSection->id, 'project' => $selectedProject->id]) }}"
                style="display: none;">
            @csrf
            @method('DELETE')
          </form>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
      <form method="POST" action="{{ route('projects.scores.save', ['subject' => $classSection->subject->id, 'classSection' => $classSection->id, 'project' => $selectedProject->id]) }}" id="scoresForm">
        @csrf
          ...
          <table class="w-full">
            <thead class="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                @endif
                <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Resubmissions</th>
              </tr>
            </thead>
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              @foreach($students as $student)
                @php
                  $studentScore = $selectedProject->getStudentScore($student->id);
                  $currentScore = $studentScore ? $studentScore->score : null;
                  $isLate = $studentScore ? $studentScore->is_late : false;
                  $resubmissionCount = $studentScore ? $studentScore->resubmission_count : 0;
                @endphp
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                    <input type="number" 
                           ...
                           placeholder="0"
                           oninput="validateScore(this, {{ $selectedProject->max_score }})">
                  </td>
                  @if($selectedProject->hasDueDate())
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                      <input type="checkbox" 
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                    <input type="number" 
                           name="resubmission_counts[{{ $student->id }}]" 
                           value="{{ $resubmissionCount }}"
                           min="0" 
                           max="10" 
                           class="w-16 px-2 py-1 text-center border border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-evsu focus:border-transparent dark:bg-gray-700 dark:text-white text-sm">
                    @if($resubmissionCount > 0)
                      <div class="text-xs text-green-600 dark:text-green-400 mt-1">
                        {{ $resubmissionCount }} resubmission{{ $resubmissionCount > 1 ? 's' : '' }}
                      </div>
                    @endif
                  </td>
                </tr>
  ...
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md mx-4 transform transition-all">
    ...
    <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
      <div class="flex items-center gap-3">
        ...
        <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100">Add New Project</h3>
      </div>
      <button onclick="closeAddProjectModal()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
        <i data-lucide="x" class="w-6 h-6"></i>
      </button>
    </div>

    <!-- Modal Body -->
    <form method="POST" action="{{ route('projects.store', ['subject' => $classSection->subject->id, 'classSection' => $classSection->id]) }}" class="p-6">
      @csrf
      <div class="space-y-4">
        <!-- Project Name -->
        <div>
  ...
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md mx-4 transform transition-all">
    ...
    <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
      <div class="flex items-center gap-3">
        ...
        <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100">Edit Project</h3>
      </div>
      <button onclick="closeEditProjectModal()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
        <i data-lucide="x" class="w-6 h-6"></i>
      </button>
    </div>

    <!-- Modal Body -->
    <form method="POST" action="{{ $selectedProject ? route('projects.update', ['subject' => $classSection->subject->id, 'classSection' => $classSection->id, 'project' => $selectedProject->id]) : '#' }}" class="p-6">
      @csrf
      @method('PUT')
      <div class="space-y-4">
        <!-- Project Name -->
        <div>
          <label for="edit_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Project Name</label>
        ...
        <div>
          ...
          <input type="number" id="edit_max_score" name="max_score" required 
                 ...
                 max="999.99" 
                 step="0.01"
                 value="{{ $selectedProject ? $selectedProject->max_score : '' }}">
        </div>
        ...
        <div>
          ...
          <input type="date" id="edit_due_date" name="due_date" 
                 class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-evsu focus:border-transparent dark:bg-gray-700 dark:text-white"
                 value="{{ $selectedProject && $selectedProject->due_date ? $selectedProject->due_date->format('Y-m-d') : '' }}">
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Leave empty if no due date</p>
        </div>
        ...
        <div>
          ...
          <textarea id="edit_description" name="description" rows="3"
                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-evsu focus:border-transparent dark:bg-gray-700 dark:text-white"
                    placeholder="Project description...">{{ $selectedProject ? $selectedProject->description : '' }}</textarea>
        </div>
      </div>
