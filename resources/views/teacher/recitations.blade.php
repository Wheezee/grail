...
<nav class="mb-6" aria-label="Breadcrumb">
  <ol class="flex flex-wrap items-center gap-1 sm:gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400">
    ...
    <li class="flex items-center">
      <i data-lucide="chevron-right" class="w-3 h-3 sm:w-4 sm:h-4 mx-1 sm:mx-2 flex-shrink-0"></i>
      <span class="text-gray-900 dark:text-gray-100 font-medium whitespace-nowrap">Recitations</span>
    </li>
  </ol>
</nav>
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
  <div>
    <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Recitations — {{ $classSection->section }}</h2>
    <p class="text-gray-600 dark:text-gray-400 mt-1">{{ $classSection->subject->code }} - {{ $classSection->subject->title }}</p>
  </div>
  <button onclick="openAddRecitationModal()" class="inline-flex items-center gap-2 px-5 py-2.5 bg-evsu hover:bg-evsuDark text-white font-semibold rounded-lg shadow transition-transform transform hover:scale-105 focus:outline-none">
...
@if($recitations->count() > 0)
  <!-- Recitation Navigation Tabs -->
  <div class="mb-6">
    <div class="flex flex-wrap gap-2 border-b border-gray-200 dark:border-gray-700">
      @foreach($recitations as $recitation)
        <div class="px-0 py-0 text-sm font-medium rounded-t-lg transition-all duration-200 flex items-center group {{ $selectedRecitation && $selectedRecitation->id === $recitation->id ? 'bg-evsu text-white' : 'text-gray-600 dark:text-gray-400 hover:text-evsu dark:hover:text-evsu hover:bg-gray-100 dark:hover:bg-gray-700' }}">
          <a href="{{ route('recitations.show', ['subject' => $classSection->subject->id, 'classSection' => $classSection->id, 'recitation' => $recitation->id]) }}" class="flex-1 px-4 py-2 flex items-center min-w-0">
            <span class="truncate">{{ $recitation->name }}</span>
          </a>
        </div>
      @endforeach
    </div>
  ...
  @if($selectedRecitation)
      ...
      <div class="flex flex-wrap items-center justify-between gap-6">
        <div class="flex flex-wrap items-center gap-6 text-sm">
          <div class="flex items-center gap-2">
            ...
            <span class="text-gray-900 dark:text-gray-100">{{ $selectedRecitation->max_score }}</span>
          </div>
          @if($selectedRecitation->description)
            <div class="flex items-center gap-2">
              <span class="font-medium text-gray-700 dark:text-gray-300">Description:</span>
              <span class="text-gray-900 dark:text-gray-100">{{ $selectedRecitation->description }}</span>
            </div>
          @endif
        </div>
        <div class="flex items-center gap-2 ml-auto">
          <button onclick="openEditRecitationModal()" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg shadow transition-transform transform hover:scale-105 focus:outline-none h-10 bg-evsu text-white hover:bg-evsuDark">
            <i data-lucide="edit-3" class="w-4 h-4"></i>
            Edit
          </button>
          <button
            onclick="if(confirm('Delete this recitation? This action cannot be undone.')) { document.getElementById('delete-recitation-form-{{ $selectedRecitation->id }}').submit(); }"
          ...
          <form id="delete-recitation-form-{{ $selectedRecitation->id }}"
                method="POST"
                action="{{ route('recitations.destroy', ['subject' => $classSection->subject->id, 'classSection' => $classSection->id, 'recitation' => $selectedRecitation->id]) }}"
                style="display: none;">
            @csrf
            @method('DELETE')
          </form>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
      <form method="POST" action="{{ route('recitations.scores.save', ['subject' => $classSection->subject->id, 'classSection' => $classSection->id, 'recitation' => $selectedRecitation->id]) }}" id="scoresForm">
        @csrf
          ...
          <table class="w-full">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              @foreach($students as $student)
                @php
                  $studentScore = $selectedRecitation->getStudentScore($student->id);
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                    <input type="number" 
                           ...
                           value="{{ $currentScore }}"
                           min="0" 
                           max="{{ $selectedRecitation->max_score }}" 
                           step="0.01"
                           ...
                           placeholder="0"
                           oninput="validateScore(this, {{ $selectedRecitation->max_score }})">
                  </td>
                </tr>
              @endforeach
            </tbody>
...
@else
  ...
  <div class="flex flex-col items-center justify-center py-24">
    ...
    <p class="text-lg text-gray-500 mb-2">No recitations found. Click 'Add Recitation' to get started.</p>
    <button onclick="openAddRecitationModal()" class="inline-flex items-center gap-2 px-5 py-2.5 bg-evsu hover:bg-evsuDark text-white font-semibold rounded-lg shadow transition-transform transform hover:scale-105 focus:outline-none">
      <i data-lucide="plus" class="w-5 h-5"></i>
      ➕ Add Recitation
    </button>
...
<div id="addRecitationModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md mx-4 transform transition-all">
    ...
    <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
      <div class="flex items-center gap-3">
        ...
        <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100">Add New Recitation</h3>
      </div>
      <button onclick="closeAddRecitationModal()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
        <i data-lucide="x" class="w-6 h-6"></i>
      </button>
    </div>

    <!-- Modal Body -->
    <form method="POST" action="{{ route('recitations.store', ['subject' => $classSection->subject->id, 'classSection' => $classSection->id]) }}" class="p-6">
      @csrf
      <div class="space-y-4">
        <!-- Recitation Name -->
        <div>
          <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Recitation Name</label>
        ...
        <div>
          ...
          <textarea id="description" name="description" rows="3"
                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-evsu focus:border-transparent dark:bg-gray-700 dark:text-white"
                    placeholder="Recitation description...">{{ old('description') }}</textarea>
        </div>
      </div>

      <!-- Modal Footer -->
      <div class="flex gap-3 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
        <button type="button" onclick="closeAddRecitationModal()" 
...
<div id="editRecitationModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md mx-4 transform transition-all">
    <!-- Modal Header -->
    <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
      <div class="flex items-center gap-3">
        <i data-lucide="edit-3" class="w-6 h-6 text-evsu"></i>
        <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100">Edit Recitation</h3>
      </div>
      <button onclick="closeEditRecitationModal()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
        <i data-lucide="x" class="w-6 h-6"></i>
      </button>
    </div>

    <!-- Modal Body -->
    <form method="POST" action="{{ $selectedRecitation ? route('recitations.update', ['subject' => $classSection->subject->id, 'classSection' => $classSection->id, 'recitation' => $selectedRecitation->id]) : '#' }}" class="p-6">
      @csrf
      @method('PUT')
      <div class="space-y-4">
        <!-- Recitation Name -->
        <div>
          <label for="edit_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Recitation Name</label>
          <input type="text" id="edit_name" name="name" required 
                 class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-evsu focus:border-transparent dark:bg-gray-700 dark:text-white"
                 placeholder="e.g., Recitation 1"
                 value="{{ $selectedRecitation ? $selectedRecitation->name : '' }}">
        </div>
        ...
        <div>
          ...
          <input type="number" id="edit_max_score" name="max_score" required 
                 ...
                 max="999.99" 
                 step="0.01"
                 value="{{ $selectedRecitation ? $selectedRecitation->max_score : '' }}">
        </div>
        ...
        <div>
          ...
          <textarea id="edit_description" name="description" rows="3"
                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-evsu focus:border-transparent dark:bg-gray-700 dark:text-white"
                    placeholder="Recitation description...">{{ $selectedRecitation ? $selectedRecitation->description : '' }}</textarea>
        </div>
      </div>

      <!-- Modal Footer -->
      <div class="flex gap-3 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
        <button type="button" onclick="closeEditRecitationModal()" 
                class="flex-1 px-4 py-2 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg font-medium transition-colors">
          Cancel
        </button>
        <button type="submit" 
...
<script>
function openAddRecitationModal() {
  document.getElementById('addRecitationModal').classList.remove('hidden');
  document.body.style.overflow = 'hidden';
}

function closeAddRecitationModal() {
  document.getElementById('addRecitationModal').classList.add('hidden');
  document.body.style.overflow = 'auto';
}

function openEditRecitationModal() {
  document.getElementById('editRecitationModal').classList.remove('hidden');
  document.body.style.overflow = 'hidden';
}

function closeEditRecitationModal() {
  document.getElementById('editRecitationModal').classList.add('hidden');
  document.body.style.overflow = 'auto';
}
